<x-layouts.app :title="__('Create Task')">
    <div class="flex h-full w-full flex-1 flex-col gap-4">
        ...
        <div class="max-w-lg rounded-xl border border-neutral-200 bg-white p-6 dark:border-neutral-700 dark:bg-zinc-900">
            <form action="{{ route('tasks.store') }}" method="POST" class="flex flex-col gap-4">
                ...
                <flux:field>
                    ...
                    <flux:input type="date" name="due_date" value="{{ old('due_date') }}" />
                    @error('due_date')
                        <flux:error>{{ $message }}</flux:error>
                    @enderror
                </flux:field>
